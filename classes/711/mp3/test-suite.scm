(let ((time-stamp "Time-stamp: <2003-08-18 12:21:58 wand>"))
  (eopl:printf
    "test-suite.scm: unified test suite for CSG 711 ~a~%"
...
;;; this is only a partial list of tests.  It doesn't even include all
;;; the tests that are in the book (yet).

; (add-test! '(mp1 example1) 'subst-test-1 '(a b ((b c) (b () d)))
;   '((a c) (a () d)))

; (add-test! '(mp1 example1) 'subst-test-2 '(a b ()) '())

; (add-test! '(mp1 example1) 'subst-test-3 '(a b (c b d a)) '(c a d a))

(add-test! '(mp1 duple) 'duple-test-1 '(1 (a b c)) '((a b c)))

(add-test! '(mp1 duple) 'duple-test-2 '(2 (a b c)) '((a b c) (a b c)))

(add-test! '(mp1 duple) 'duple-test-0 '(0 (a b c)) '())

; (add-test! '(mp1 invert) 'invert-test-empty-list '(()) '())

; (add-test! '(mp1 invert) 'invert-test-1
;   '(((a b) (c d) (e f)))
;   '((b a) (d c) (f e)))

(add-test! '(mp1 product) 'product-test-1 
  '((a b) (c d))
  '((a c) (a d) (b c) (b d)))

(add-test! '(mp1 product) 'product-test-different-orders-1 '((a b) (c d))
  '((a c) (b c) (a d) (b d)))

(add-test! '(mp1 product) 'product-test-empty-1st-arg '(() (c d)) '())

(add-test! '(mp1 product) 'product-test-empty-2nd-arg '((a b) ()) '())

(add-test! '(mp1 product) 'product-test-singleton-1st-arg
  '((a) (b c d))
  '((a b) (a c) (a d)))

(add-test! '(mp1 product) 'product-test-singleton-2nd-arg
  '((a b c) (d))
  '((a d) (b d) (c d)))

(add-test! '(mp1 product) 'product-test-different-length-args
  '((a b) (c d e))
  '((a c) (a d) (a e)
    (b c) (b d) (b e)))
...
(add-test! 'base 'pgm3-1-1 "1" 1)

;; this should raise an error
(add-test! 'base 'pgm3-1-2 "add1(x)" 'error)
(add-test! 'base 'test3-1-unbound-variable "foo" 'error)

; for 3352, init-env is empty!
; (add-test! 'base 'test3-1-primitive-app-2 "+(x,3)" 13)
; (add-test! 'base 'nested-prim-app "+(i,*(3,4))" 13)

(add-test! 'base 'true-exp "true" #t)

(add-test! 'base 'if-true "if zero?(0) then 3 else 4" 3)

(add-test! 'base 'if-false "if zero?(1) then 3 else 4" 4)

(add-test! 'let 'pgm5 "let x = 3 y = 4 in +(x,y)" 7)

(add-test! 'let 'let-1 "let x = 5 y = 6 in +(x,y)" 11)

(add-test! 'base 'let-to-proc-1 "(proc (x,y) +(x,y) 5 6)" 11)

(add-test! 'let 'nested-let-1 "
let z = 5 x = 3
in let x = 4
       y = +(x,z)         % here x = 3
   in *(z, +(x,y))        % here x = 4" 60)

(add-test! 'base 'nested-let-as-proc "
(proc (z,x)
 (proc (x,y) *(z, +(x,y)) 4 +(x,z))
 5 3)
" 60)

(add-test! 'let 'nested-let-3 "let z = let x = 3 in *(x,x) in z" 9)

(add-test! 'let 'bind-a-proc "let f = proc(x,y)+(x,y) in (f 3 4)" 7)

(add-test! 'base 'apply-a-proc "(proc(x,y)-(x,y)  4 3)" 1)


(add-test! 'let 'infinite-loop "
let fix =  proc (f)
...
(add-test! 'print 'easy-print-1 "print(3)" 1)
(add-test! 'print 'easy-print-2 "let x = 3 in print(*(x,3))" 1)

(add-test! 'print 'check-eval-order "
let a = print(1)
    b = print(2)
in +(a,b)"
2)

(add-test! 'varassign1 'gensym-test-1 
"let g = let counter = ref(0) 
         in proc () let d = setref(counter, add1(deref(counter)))
                    in deref(counter)
in +((g),(g))" 3)
                  

(add-test! 'varassign1 'even-odd-via-set-1 "
let x = ref(0)
...
(add-test! 'lang3-7-explicit 'gensym-explicit "
let g = let counter = ref(0) 
         in proc () let d = setref(counter, add1(deref(counter)))
                    in deref(counter)
in +((g),(g))" 
...
(add-test! 'lang3-7-explicit 'even-odd-via-set-explicit "
let x = ref(0)
in letrec even() = if zero?(deref(x)) 
                   then 1
                   else let d = setref(x, sub1(deref(x)))
...
let fix =  proc (int f)
    ...
    t4m = proc (int f, int x) if zero?(x) then 0 else +(4,(f -(x,1)))
in let times4 = (fix t4m)
...
in let d = spawn set acc = 20
   in letrec 
        loop () = if acc   %% needs to be fixed for com3352
...
in let d1 = spawn set buf1 = 20
       ...
       d3 = spawn letrec 
                  ...
                  in (loop)
   in letrec
        loop () = if buf3 then buf3 else (loop)  % fix for 3352
      in (loop)
"
  24)
