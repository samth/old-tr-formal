...
;; numeric computations, but with many primitives
    ...
    (expression ("nil?" expression) nil-test-exp)
   ...
   (let recur ((exp exp)
     ...
     (letrec
       ((recur*                         ; handy mapping function
        ...
        (do-let*-subst
         (lambda (rhss ids bvars)
           (cond
             ((null? rhss) '())
             (else (cons (recur (car rhss) bvars) (do-let*-subst (cdr rhss) (cdr ids) (cons (car ids) bvars))))))))
...
;             (recur letrec-body (append proc-names bvars))))
         ...
         (cond-exp 
          (tests results)
          (cond-exp (recur* tests bvars)  (recur* results bvars))) 
         (nil-exp () (nil-exp))
         (cons-exp (arg1 arg2)
                   (cons-exp (recur arg1 bvars) (recur arg2 bvars)))
         (cdr-exp (l)
                  (cdr-exp (recur l bvars)))
         (car-exp (l)
                  (car-exp (recur l bvars)))
         (nil-test-exp (l)
                       (nil-test-exp (recur l bvars)))
         
         (let*-exp (ids rhss body)
                   (cond ((null? ids) (let*-exp ids rhss (recur body bvars)))
                         (else (let*-exp
                                ids 
                                (do-let*-subst rhss ids bvars) 
                                (recur body (append ids bvars)))))) 
