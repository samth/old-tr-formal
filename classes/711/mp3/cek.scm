...
;; environment-passing, context-passing interpreter for base language

(let ((time-stamp "Time-stamp: <2003-09-18 23:30:09 wand>"))
  (eopl:printf
    "base/context.scm ~a~%"
    (substring time-stamp 13 29)))

;;; As with the environment machine, we don't want to have to think
;;; about evaluating intermediate forms like closures.  So we make the
;;; values DISTINCT from the expressions.

;;; This could have been done as early as bigstep.scm, but I'm not
;;; sure whether it's motivated there.

;;; now we have value-configs, value*-configs
...
(define reduce-to-value-program
  (lambda (pgm)
    (cases program pgm
      (a-program (exp)
        (reduce-to-value
          (a-config exp (empty-env) (empty-context)))))))
