(let ((time-stamp "Time-stamp: <2003-08-18 12:21:39 wand>"))
  (eopl:printf "test-harness.scm: unified test harness for CSG 711 ~a~%" (substring time-stamp 13 29)))
...
;; a better organization would maybe have avoided so much duplication
;; of code.
...
(define mp1:question->experiment
  (lambda (q)
    (case q
      ...
      ((leaf-sum) (lambda (w) (apply leaf-sum w)))
      ((subst-in-tree) (lambda (w) (apply subst-in-tree w)))
