...
This means that in 
environment $\Gamma$ (which serves as both the type and the
proposition environment), \e{} has the type \t{}, that evaluating \e{} and
...
\s{r} = \applyobject{\movervec[i]{\s{a}}}{\sh{}}
 % \left\{
 %  \protect{\begin{array}{ll}
...
\inferrule[T-If]
{
  ...
  \\\\
  \hastyeffphi[\G{} \cup {\op1}]{\e2}{\t2}{\phii2} {\s2}
  \\\\
  \hastyeffphi[\G{} \cup {\op2}]{\e3}{\t3}{\phii3} {\s3}
  \\\\
  \subtype{\t2}{\t{}} \\
  \subtype{\t3}{\t{}} \\\\
...
\qquad \qquad
\inferrule[L-Update]
{\Gamma \vdash \t{\wpi{\x{}}} \\ \Gamma \vdash \t{\x{}}'}
{\Gamma \vdash {\update[\t{}]{\pi}{\t{}'}}_{\x{}}}
\fi
\]
...
%        \end{array}} \right.
    \abstractobject{\movervec[i]{\x{}}}{\s{}} & \pathmaybe{\pii} &
    \text{ if } \s{} = \wpi{\xi} \\
...
\vspace{2mm}\\
    ...
    \applyobject{\movervec[i]{\s{a}}}{\sh{}}  &      \wpip{\wpi{x}}
    & \text{ if } \sh{} = \pathmaybe{\pipi} \text{ and } {\sai} = \wpi{\x{}} \\
