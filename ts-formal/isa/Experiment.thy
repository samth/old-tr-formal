...
ML {* fun v' (terms ) avoid fix rules facts = 
  let val insts = [ (map (fn (x) => SOME (NONE, x)) terms)]
  val avoid = map (fn x => ("",x)) avoid
  val fix = [map (fn x => ("",x)) fix]
  in
  NominalInduct.nominal_induct_tac @{context} insts avoid fix rules facts 1
  end*}
...
where
"appendl ([]) = []"
| "appendl ((x::'a list)#xs) = append x (appendl xs)"


fun flatten ::  "ITree \<Rightarrow> nat list"
where
"flatten (Leaf n) = [n]"
