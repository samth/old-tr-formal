...
To see the problem, consider a system with a class \txt{Eagle} and an
instance \txt{Harry}.  The static type system can ensure that any
messages that \txt{Harry} responds to are also understood by any other
instance of \txt{Eagle}.  
...
\txt{Eagle.isEndangered()}

However, if \txt{Salmon} is a species, our type system provides us no
way of requiring that it also have an \txt{isEndangered} method.  The
...
\txt{Harry} and classes such as \txt{Eagle}.
...
{\txt{typeOf}} because it is the type of the expression that is its
argument.  
...
definitions of \txt{Species}, \txt{theEagle}, etc. above.  
...
If we want to perform the call {\txt{foo(harry)}}, then we have two
choices for the bound \txt{X} on {\txt T}.  It must be either \Object,
in which case {\txt{m}} must be a method that \Object\
...
{\txt{Pair}} class.  There are two methods: a class method
