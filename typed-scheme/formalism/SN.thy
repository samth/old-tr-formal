(* $Id: SN.thy,v 1.22 2007/02/07 16:52:34 berghofe Exp $ *)
...
lemma typing_induct_complete[consumes 1, case_names t1 t2 t3]:
  ...
  and    x :: "'a::fs_name"
  assumes a: "\<Gamma> \<turnstile> t : \<tau>"
  and a1:    "\<And>\<Gamma> (a::name) \<tau> x. valid \<Gamma> \<Longrightarrow> (a,\<tau>) \<in> set \<Gamma> \<Longrightarrow> (!! x t T \<Gamma>. (t,Var a) : smaller_than \<Longrightarrow> \<Gamma> \<turnstile> t : T \<Longrightarrow> P x \<Gamma> t T) 
  \<Longrightarrow> P x \<Gamma> (Var a) \<tau>"
  and a2:    "\<And>\<Gamma> \<tau> \<sigma> t1 t2 x. (!! x t T \<Gamma>. (t,App t1 t2) : smaller_than \<Longrightarrow> \<Gamma> \<turnstile> t : T \<Longrightarrow> P x \<Gamma> t T) \<Longrightarrow>
              \<Gamma> \<turnstile> t1 : \<tau>\<rightarrow>\<sigma> \<Longrightarrow> (\<And>z. P z \<Gamma> t1 (\<tau>\<rightarrow>\<sigma>)) \<Longrightarrow> \<Gamma> \<turnstile> t2 : \<tau> \<Longrightarrow> (\<And>z. P z \<Gamma> t2 \<tau>)
              \<Longrightarrow> P x \<Gamma> (App t1 t2) \<sigma>"
  and a3:    "\<And>a \<Gamma> \<tau> \<sigma> t x. a\<sharp>x \<Longrightarrow> a\<sharp>\<Gamma> \<Longrightarrow> ((a,\<tau>) # \<Gamma>) \<turnstile> t : \<sigma> \<Longrightarrow> (\<And>z. P z ((a,\<tau>)#\<Gamma>) t \<sigma>)
  \<Longrightarrow> (!! x t' T \<Gamma>. (t',Lam[a].t) : smaller_than \<Longrightarrow> \<Gamma> \<turnstile> t' : T \<Longrightarrow> P x \<Gamma> t' T) 
  \<Longrightarrow> P x \<Gamma> (Lam [a].t) (\<tau>\<rightarrow>\<sigma>)"
  shows "P x \<Gamma> t \<tau>"
  using a
