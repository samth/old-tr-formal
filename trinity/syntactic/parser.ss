    ...
    (parser
     ...
     (grammar
      ...
      (Literal
       ...
       [(STRING_LIT) (make-string-literal (build-src 1) (car $1))]
      ...
      (CompilationUnit
       ...
       [()
        (make-compilation-unit (make-src 0 0 0) #f null null)])
    ...
    (cond
      [(string? input)
       (let ([input-port (open-input-string input)])
         ...
         (parse input-port))]
      [(input-port? input)
       (java-parser (lambda () (java-lexer input)))]
      [else (error "huh?")]))
  
  (define-syntax parser-test
    (syntax-rules ()
      [(_ str)
       (make-test-case (substring str 0 (min 10 (string-length str)))
        (assert-not-exn (lambda () (parse str))))]))
  
  (define-syntax parser-fail-test
    (syntax-rules ()
      [(_ str)
       (make-test-case (string-append "Fail: " (substring str 0 (min 10 (string-length str))))
        (assert-exn (lambda x #t) (lambda () (parse str))))]))
